...
def weigth_loss(s, t, na=0.0125):
    # return label weight
    s = s.detach().cpu().numpy()
    t = t.detach().cpu().numpy()
    result = s * t * math.exp(s * t * na)
    # print("!!!!!!!!!!!!!!!!!!!!!!!!!!")
    result = 1.0 - torch.Tensor([result])
    return result
    ...
    def __call__(self, predictions, targets, imgs):
        ...
        for i, prediction in enumerate(predictions):
            ...
            if n:
                ...
                cls_score = torch.max(F.softmax(prediction_pos[:, 5:], dim=1), 1)  # cgb
                cls_score_mean = torch.mean(cls_score[1].float())
                weight_a = weigth_loss(iou.mean(), cls_score_mean).cuda()
                # print(iou.shape)
                # -------------------------------------------#
                ...
                # print(torch.max(prediction_pos[:, 5:], dim=1))
                box_loss += weight_a * (1.0 - iou).mean()  # iou weight
                # box_loss += (1.0 - iou).mean()
                # print(iou.mean())
                # print(iou.mean().shape)
                # print(prediction_pos[:, 5:].shape)
                cls_loss += weight_a * self.BCEcls(prediction_pos[:, 5:], t)  # BCE weight
